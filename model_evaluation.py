...
class ModelEvaluator:
    ...
    def create_three_model_feature_importance(self, feature_importance_dict, save_path="results/"):
        """
        Figure: Feature Importance Comparison - RF, GB, and XGBoost
        
        Creates a three-panel horizontal bar chart comparing the top 10 features
        across Random Forest (Tuned), Gradient Boosting, and XGBoost (Tuned).
        """
        print("\n" + "="*80)
        print("GENERATING: Three-Model Feature Importance Comparison")
        print("="*80)
        
        # Extract importance data for each model
        models_data = []
        
        # 1. Random Forest (Tuned)
        rf_key = next((k for k in feature_importance_dict.keys() 
                      if 'Random Forest' in k and 'Tuned' in k), None)
        if rf_key:
            models_data.append({
                'data': feature_importance_dict[rf_key].head(10),
                'title': '(a) Random Forest (Tuned)',
                'color': 'Greens_r'
            })
            print(f"✓ Found Random Forest (Tuned)")
        
        # 2. Gradient Boosting
        if 'Gradient Boosting' in feature_importance_dict:
            models_data.append({
                'data': feature_importance_dict['Gradient Boosting'].head(10),
                'title': '(b) Gradient Boosting',
                'color': 'Blues_r'
            })
            print(f"✓ Found Gradient Boosting")
        
        # 3. XGBoost (Tuned)
        xgb_key = next((k for k in feature_importance_dict.keys() 
                       if 'XGBoost' in k and 'Tuned' in k), None)
        if xgb_key:
            models_data.append({
                'data': feature_importance_dict[xgb_key].head(10),
                'title': '(c) XGBoost (Tuned)',
                'color': 'Oranges_r'
            })
            print(f"✓ Found XGBoost (Tuned)")
        
        if len(models_data) == 0:
            print("⚠️  ERROR: No models found for feature importance")
            return
        
        n_models = len(models_data)
        print(f"Creating figure with {n_models} model(s)...")
        ...
        for idx, model_info in enumerate(models_data):
            ax = axes[idx]
            importance_df = model_info['data']
            colors_palette = sns.color_palette(model_info['color'], len(importance_df))
            
            # Horizontal bar chart
            ax.barh(range(len(importance_df)), importance_df['importance'], 
                   color=colors_palette, edgecolor='white', linewidth=0.5)
            ax.set_yticks(range(len(importance_df)))
            ax.set_yticklabels(importance_df['feature'], fontsize=9)
            ax.set_xlabel('Relative Importance', fontsize=9, fontweight='bold')
            ax.set_title(model_info['title'], fontsize=10, fontweight='bold', pad=10)
            ax.invert_yaxis()
        ...
        plt.savefig(f'{save_path}enhanced_feature_importance_three_models.pdf', 
                   dpi=300, bbox_inches='tight')
        plt.savefig(f'{save_path}enhanced_feature_importance_three_models.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"\n✅ SAVED: {save_path}enhanced_feature_importance_three_models.pdf")
        print(f"✅ SAVED: {save_path}enhanced_feature_importance_three_models.png")
    ...
    def create_dual_prediction_plots(self, results, save_path="results/"):
        """
        Figure: Predicted vs Actual Revenue - XGBoost and Gradient Boosting
        
        Creates side-by-side scatter plots comparing prediction accuracy
        of the two best-performing models.
        """
        print("\n" + "="*80)
        print("GENERATING: Dual Prediction Accuracy Plots")
        print("="*80)
        
        # Find the two models
        xgb_key = next((k for k in results.keys() 
                       if 'XGBoost' in k and 'Tuned' in k), None)
        gb_key = 'Gradient Boosting' if 'Gradient Boosting' in results else None
        
        if not xgb_key:
            print("⚠️  ERROR: XGBoost (Tuned) not found in results")
            return
        if not gb_key:
            print("⚠️  ERROR: Gradient Boosting not found in results")
            return
        ...
        for model in models:
            ...
            # Scatter plot
            ax.scatter(y_test_m, y_pred_m, alpha=0.6, s=25, 
                      color=model['color'], edgecolors='white', linewidth=0.3)
            
            # Perfect prediction line
            max_val = max(y_test_m.max(), y_pred_m.max())
            min_val = min(y_test_m.min(), y_pred_m.min())
            ax.plot([min_val, max_val], [min_val, max_val], 'r--', 
                   linewidth=2.5, label='Perfect Prediction', zorder=5, alpha=0.8)
            
            # ±20% error bounds
            ax.fill_between([min_val, max_val], 
                           [min_val*0.8, max_val*0.8], 
                           [min_val*1.2, max_val*1.2],
                           alpha=0.15, color='gray', label='±20% Error Band', zorder=1)
            
            # Labels and formatting
            ax.set_xlabel('Actual Revenue (Million USD)', fontsize=10, fontweight='bold')
            ax.set_ylabel('Predicted Revenue (Million USD)', fontsize=10, fontweight='bold')
            ax.set_title(f'{model["title"]}\nR² = {r2:.4f}, RMSE = ${rmse/1e6:.1f}M, MAE = ${mae/1e6:.1f}M',
        ...
        # Save in multiple formats
        plt.savefig(f'{save_path}enhanced_prediction_accuracy_dual.pdf', 
                   dpi=300, bbox_inches='tight')
        plt.savefig(f'{save_path}enhanced_prediction_accuracy_dual.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"\n✅ SAVED: {save_path}enhanced_prediction_accuracy_dual.pdf")
        print(f"✅ SAVED: {save_path}enhanced_prediction_accuracy_dual.png")
    ...
    def comprehensive_evaluation_springer(self, results, feature_importance_dict=None, 
        ...
        if feature_importance_dict:
            self.create_three_model_feature_importance(feature_importance_dict, save_path)
        ...
        self.create_dual_prediction_plots(results, save_path)
